...
import { BarChart3, TrendingUp, TrendingDown, Clock, CheckCircle, XCircle, RefreshCw, Trash2 } from "lucide-react"
...
interface MetricsSummary {
  ...
  requestsPerEndpoint: Record<string, { count: number; success: number; avgTime: number }>
}
...
function calculateSummary(metrics: ApiMetrics[]): MetricsSummary {
  ...
  const byEndpoint: Record<string, { count: number; success: number; totalTime: number }> = {}
  metrics.forEach((m) => {
  ...
  const requestsPerEndpoint: Record<string, { count: number; success: number; avgTime: number }> = {}
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <Button variant="ghost" size="icon" onClick={handleClear} disabled={metrics.length === 0}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-lg bg-secondary/50 border border-border">
            ...
            <p className="text-2xl font-bold">{summary.p95ResponseTime.toFixed(0)}ms</p>
          </div>
        </div>
        ...
        {metrics.length > 0 && (
          <div className="p-3 rounded-lg bg-secondary/30 border border-border">
            <p className="text-xs text-muted-foreground mb-2">Response Time Range</p>
            <div className="flex items-center justify-between text-sm">
              <span className="text-success">{summary.minResponseTime.toFixed(0)}ms</span>
              ...
              <span className="text-destructive">{summary.maxResponseTime.toFixed(0)}ms</span>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-muted-foreground">By Endpoint</h4>
          
          {Object.keys(summary.requestsPerEndpoint).length === 0 ? (
            <div className="text-center py-6 text-muted-foreground">
              <BarChart3 className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p className="text-sm">No metrics yet</p>
              <p className="text-xs mt-1">Make API requests to see performance data</p>
            </div>
          ) : (
            <div className="space-y-2">
              {Object.entries(summary.requestsPerEndpoint).map(([endpoint, data]) => {
                const successRate = (data.success / data.count) * 100
                return (
                  <div key={endpoint} className="p-2 rounded-lg border border-border bg-card/50">
                    <div className="flex items-center justify-between mb-2">
                      <code className="text-xs font-mono truncate flex-1">{endpoint}</code>
                      <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="text-xs">
                          {data.count} req
                        </Badge>
                        <Badge 
                          variant={successRate >= 90 ? "success" : "destructive"} 
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-secondary rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(data.avgTime / maxAvgTime) * 100}%` }}
                        />
                      </div>
