...
export function initTracing() {
  ...
  if (OTEL_EXPORTER_URL) {
    ...
    console.log(
      `[OpenTelemetry] Configured OTLP exporter: ${OTEL_EXPORTER_URL}`,
    );
  } else {
    console.warn(
      "[OpenTelemetry] No exporter URL configured. Traces will not be exported.",
    );
  }
...
export function startSpan(
  name: string,
  fn: (span: Span) => void | Promise<void>,
): void | Promise<void> {
